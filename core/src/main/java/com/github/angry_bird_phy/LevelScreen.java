...
public class LevelScreen implements Screen, ContactListener {
    ...
    public GameState saveGameState() {
        ...
        for (Pig pig : pigList) {
            ...
            gameState.pigList.add(new HittableState(
                    ...
                    pig.getHealth(), pig.body.getAngle() * MathUtils.radiansToDegrees));
        }

        // Save blocks
        for (Block block : blockList) {

            // System.out.println(block.body.getAngle() * MathUtils.radiansToDegrees);
            // float blkscale = block.Block_Scale;
            ...
            gameState.blockList.add(new HittableState(
                    block.body.getPosition().x,
                    block.body.getPosition().y,
                    block.getHealth(), block.body.getAngle() * MathUtils.radiansToDegrees));
        }
    ...
    public void loadGameState(GameState gameState) {
        ...
        for (HittableState pigState : gameState.pigList) {
            Pig pig = new NormalPig(world, this, 0, 0); // Example for creating a pig
        ...
        for (HittableState blockState : gameState.blockList) {
            Block block = new WoodenBlock(world, this, 0, 0); // Example for creating a block
    ...
    public void addAction(Runnable action, float delay) {
        Timer.schedule(new Timer.Task() {
            @Override
            public void run() {
                action.run();
